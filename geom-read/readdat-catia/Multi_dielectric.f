...
!      tmp_filename1=trim(Path)//trim(Dataname);
      ...
      tmp2 = 0
      do while(iFlag .ne. 1)
         read(Uinp,12101) tmp_char1
c         if(trim(tmp_char1) .eq. '*') then
            tmp2 = tmp2+1
            read(Uinp,*) tmp_char1
...
800   tmp1 = 0
     ...
     &               (trim(tmp_char1).ne.'CTETRA')))
      ...
      if(trim(tmp_char1) .eq. 'CTRIA3') then
         iNum(1) = iNum(1) + 1
         if(iNum(1) .gt. 20) then
            write(*,*) "Ipat-Element type is larger than 20!"
            stop
         endif
...
600   read(Uinp,*) tmp_char1
      ...
      iNum(2) = iNum(2) + 1      
      if(iNum(1) .gt. 20) then
        write(*,*) "Tetra-Element type is larger than 20!"
        stop
      endif
      ...
      do i=1,iNum(2)
        write(*,*),"type and max:tetra =",i+iNum(1),tetra(i)
      enddo
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      allocate(rp(nsurfem+maxpatch),iqof(nsurfem+maxpatch),
     &            iqfo(nsurfem+maxpatch))
      allocate(ip(nsurfem+maxpatch),iq(nsurfem+maxpatch))
      allocate(fface(4,nsurfem+maxpatch))
      nt=0
      do i=1,nsurfem
         nt=nt+1
         fface(1,nt)=ipat(1,i)
      ...
      do 301 i=1,facet
         if(iqfo(i).le.nsurfem)then
            do i1=1,i-1
               i2=i-i1
            if(i2.ge.1)then
               if(abs(rp(i2)-rp(i)).gt.0.01) goto 302
               j1 = fface(1,iqfo(i))
               j2 = fface(2,iqfo(i))
               j3 = fface(3,iqfo(i))
               call comi(j1,j2)
               ...
               call comi(k1,k3)
               call comi(k2,k3)
               if((j1.eq.k1).and.(j2.eq.k2).and.
     &                              (j3.eq.k3))then
               if(iqfo(i2).gt.nsurfem)then
                  nt=nt+1
                  ip(nt)=iqfo(i)
                  iq(nt)=iqfo(i2)
                  goto 301
...
302         do i1=1,facet-i
               i2=i+i1
            if(i2.le.facet)then
               if(abs(rp(i2)-rp(i)).gt.0.01) goto 301
               j1 = fface(1,iqfo(i))
               j2 = fface(2,iqfo(i))
               j3 = fface(3,iqfo(i))
               call comi(j1,j2)
...
301   continue
      ...
      do i=1,nt
         i1=ip(i)
         i2=iq(i)
         lva(ipat(5,i1),ipat(4,i1))=fface(4,i2)
         if(fface(4,i2).eq.10)then
            j1=j1+1
         endif
         if(fface(4,i2).eq.9)then
            j2=j2+1
         endif
      enddo
      print*,'ABC:',j1
